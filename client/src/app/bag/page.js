...
const Bag = () => {
  ...
  const {
    ...
    data: petDataNega,
    error,
  } = useFetch("?category=bag&type=negative");

  const {
    loading: loadingPos,
    data: petDataPos,
    error: errorPos,
  } = useFetch("?category=bag&type=positive");
  ...
  return (
    <div>
      ...
      >
        ...
        <option value={"fanny-bag"}>Túi đeo chéo</option>
        <option value={"laptop-bag"}>Túi laptop</option>
      </select>
      <select
        name="category"
        id="category"
